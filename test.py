...
# ==============================
# Google Books API (온라인 추천)
# ==============================
@st.cache_data(show_spinner=False, ttl=3600)
def recommend_by_keyword_api(keyword: str, max_results: int = 6) -> List[Dict]:
    """
    Google Books에서 키워드로 책 검색 (표지/설명 포함)
    - 한국어 우선, 결과 없으면 언어 제한 없이 재시도
    """
    def _fetch(q: str, lang: str | None):
    ...
    # 1) ko 우선
    recs = _fetch(keyword, "ko")
    # 2) 부족하면 언어 제한 해제
    if len(recs) == 0:
        recs = _fetch(keyword, None)
...
with st.sidebar:
    ...
    menu = st.radio(
        "메뉴",
        ["홈", "성향 테스트", "장르별 추천", "내 리스트 기반(간단)", "키워드 온라인 추천"],
        index=0
    )
    st.caption("한글 10유형 기반 + Google Books 온라인 추천")
...
if menu == "홈":
    ...
    with cols[2]:
        st.subheader("③ 온라인 추천")
        st.write("키워드 기반으로 Google Books에서 실시간 추천")
...
# ------------------------------
    ...
    if st.button("결과 보기 ✨", type="primary"):
        if any(a == "" for a in answers):
            ...
            for b in recs:
                st.markdown(
                    f"<div class='card'><div><b>{b['title']}</b> · {b['author']}<br>"
                    f"<span class='small-note'>추천 이유: {b['reason']}</span></div></div>",
                    unsafe_allow_html=True,
                )
...
# ------------------------------
    ...
    if st.button("온라인 추천 보기"):
        if not keyword.strip():
            ...
            if not recs:
                st.info("관련 도서를 찾지 못했습니다. 다른 키워드를 시도해 보세요.")
            else:
                for b in recs:
                    thumb_html = f"<img src='{b['thumbnail']}' alt='thumb'>" if b.get("thumbnail") else ""
                    desc = (b.get("description") or "설명 없음").strip()
                    meta = []
                    if b.get("publishedDate"): meta.append(b["publishedDate"])
                    if b.get("categories"): meta.append(b["categories"])
                    meta_str = " · ".join(meta)
                    st.markdown(
                        f"<div class='card'>{thumb_html}"
                        f"<div><b>{b['title']}</b><br>"
                        f"저자: {b['authors']}" + (f" <span class='small-note'>({meta_str})</span>" if meta_str else "") +
                        f"<br><span class='small-note'>{desc}</span></div></div>",
                        unsafe_allow_html=True,
                    )
